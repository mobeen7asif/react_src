...
class MemberJoinDate extends Component {

    memberJoin = ['On', 'Before', 'After', 'Between'];
    ...
    setCriteriaValue = (value) => {
        this.props.setCriteriaValue('creation_datetime', 'value', value);
        ...
        return (
            <div className="dropSegmentation_section">
                <div className="dropSegmentation_heading clearfix">
                    <h3>Membership Join Date <span style={{fontSize: '10px',marginLeft: '19px',fontStyle: 'italic', color: 'silver'}}>We are using Australian Eastern Standard Time (AEST), please set your time accordingly</span></h3>
                    <div className="segmntClose" onClick={(e)=> {this.props.removeCriteria('creation_datetime')}}>
                        <a  style={{cursor:'pointer'}}>&nbsp;</a>
                    </div>
                </div>
                <div className="stateSegmentation">
                    <div className="compaignDescription_outer clearfix">
                        <label>Member join date is</label>
                        <div className="placeHolderOuter expandPlaceholder clearfix">
                            <div className="customDropDown">
                                <span ref={ref => this.customDropDownSpanRef = ref} onClick={this.handleDropDownSpanClick}> {this.props.criteria.value.memberJoin ? this.props.criteria.value.memberJoin : 'Membership Join'} </span>
                                <ul className="customDropDown_show customPlaceHolder" ref={ref => this.customDropDownShowRef = ref} style={{marginBottom: '30px',marginTop: '-10px', maxHeight:'207px', display:'none'}} >
                                    <Scrollbars autoHeight renderTrackHorizontal={() => <div></div>} renderThumbHorizontal={() => <div></div>}>
                                        {
                                            this.memberJoin.map((ms) => {
                                                return <li key={ms} onClick={(e)=> {this.setValueSelected(ms)}}
                                                           className={(this.props.criteria.value.memberJoin === ms) ? 'selectedItem' : ''}>{ms}</li>
                                            })
                                        }
                                    </Scrollbars>
                                </ul>
                        ...
                        <div className="datePickerOuter clearfix">
                            ...
                            <div className="datePickerLeft frDatePicker" style={{display: (this.props.criteria.value.memberJoin === 'Between' ? 'block' : 'none')}}>
                                <strong>And</strong>
                                <div className="datePicker">
                                    <DatePicker selected={this.state.toDate} dateFormat="DD MMM YYYY"  onChange={this.handleChangeEndDate}/>
                                </div>
                            </div>
                        </div>
                    </div>

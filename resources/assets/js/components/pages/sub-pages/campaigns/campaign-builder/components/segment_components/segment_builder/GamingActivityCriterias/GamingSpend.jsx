...
class GamingSpend extends Component {
    gamingSpend    = ['More Than', 'Less Than', 'Equal To', 'Not Equal To', 'Between'];
    ...
    setCriteriaValue = (value) => {
        this.props.setCriteriaValue('gaming_spend', 'value', value);
    ...
    setValueSelected = (baseKey, key, value) => {
        ...
        // preValue = {gaming_spend: {condition: 'between', v1: 2324, v2: 34435}, transaction_date: {condition: '', fromDate: '', toDate: ''}}
    ...
    setGamingSpendValue = (e, key, index) => {
        ...
        if (! isNaN(value)) {
            this.setState((prevState) => {
                ...
                let preVal = this.props.criteria.value;
                preVal.gaming_spend[key] = this.state[key].join('');
                this.setCriteriaValue(preVal);
            });
        }
    ...
    initializeStateValues = () => {
        ...
        if (this.props.criteria.value.gaming_spend.v1) {
            this.setState(() => {
                let v1 = Array(8);
                this.props.criteria.value.gaming_spend.v1.split('').map((val,index) => {
                    v1[index] = val;
                });
                return {v1: v1};
            });
        ...
        if (this.props.criteria.value.gaming_spend.v2) {
            this.setState(() => {
                let v2 = Array(8);
                this.props.criteria.value.gaming_spend.v2.split('').map((val,index) => {
                    v2[index] = val;
                });
                return {v2: v2};
            });
        ...
        return (
            <div className="dropSegmentation_section">
                <div className="dropSegmentation_heading clearfix">
                    <h3>Gaming Spend</h3>
                    <div className="segmntClose" onClick={(e)=> {this.props.removeCriteria('gaming_spend')}}>
                        <a  style={{cursor:'pointer'}}>&nbsp;</a></div>
                </div>
                <div className="stateSegmentation">
                    <div className="compaignDescription_outer clearfix">
                        <label>Gaming Spend is</label>
                        <div className="gammingTurnOuter clearfix">
                            <div className="gammingTurn_placeholder">
                                <div className="customDropDown">
                                    <span onClick={this.handleDropDownSpanFirstClick} ref={ref => this.customDropDownFirstSpanRef = ref}> {this.props.criteria.value.gaming_spend.condition ? this.props.criteria.value.gaming_spend.condition : 'Gaming Spend On'}</span>
                                    <ul className="customDropDown_show customPlaceHolder" ref={ref => this.customDropDownShowFirstRef = ref} style={{marginBottom: '30px',marginTop: '-10px', maxHeight:'207px', display:'none'}} >
                                        <Scrollbars autoHeight renderTrackHorizontal={() => <div></div>} renderThumbHorizontal={() => <div></div>}>
                                            {
                                                this.gamingSpend.map((ptype) => {
                                                    return <li key={ptype} onClick={(e)=> {this.setValueSelected('gaming_spend', 'condition', ptype)}}>{ptype}</li>
                                                })
                                            }
                                        </Scrollbars>
                                    </ul>
                        ...
                        <div className="gammingTurnOuter clearfix"  style={{display: this.props.criteria.value.gaming_spend.condition === 'Between' ? 'block' : 'none'}}>
                            <div className="gammingValue_outer clearfix">
                                <small>$</small>
                                <div className="gamingAmount clearfix">
